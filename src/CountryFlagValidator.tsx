...
const CountryFlagValidator: React.FC<Props> = ({ password }) => {
    // Stav pro uložení jednoho konkrétního státu, aby se neměnil při psaní!
    const [selectedCountry, setSelectedCountry] = useState<string>("");

    useEffect(() => {
        // Vybíráme náhodný stát JEN JEDNOU, když se komponenta objeví
        const randomCountry = countries[Math.floor(Math.random() * countries.length)];
        setSelectedCountry(randomCountry);
    }, []);

    // Ochrana: Dokud se nevybere stát, nevykresluj nic
    if (!selectedCountry) return null;

    // Validace (převedeme obojí na malá písmena, aby to nebylo case-sensitive)
    const isValid = password.toLowerCase().includes(selectedCountry.toLowerCase());

    // Načítání vlajky pomocí spolehlivého API (FlagCDN vyžaduje malá písmena)
